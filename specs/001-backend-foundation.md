...
| DR-03 | `Scene` has the Figure 2 fields — `id`, `pov`, `story_time: int` (hours since `epoch_zero`, Decision 8), `discourse_order: int`, `location`, `goal`, `conflict`, `outcome` (four-value enum), `value_change`, `entry_state`, `exit_state`, `tags`, `budget` — and `participants: list[str]` (Decision 9). |
...
| FR-IDX-02 | One row per entity in `canon/`, `cast/` and `manuscript/digests/` (`entity_id`, `kind`, `path`, `text`, `content_hash`, `updated_at`) and an FTS5 table over `text`. |
...
| FR-AGENT-09 | All roles | No role receives a tool that reads the tree: reads are done by the orchestrator through the store layer and handed in as documents. The only tools a role holds are the writes FR-PERM-06 derives. |
...
| FR-TURN-04 | Promotion happens inside the turn, before it returns, so the next scene's assembly sees the new facts. Collided facts are left `pending` with `conflict: true`; the turn ends `awaiting_ruling` and reports them. |
...
| FR-TURN-07 | A turn record is written to `.index/turns/NNN-<n>.yaml`: scene, started/ended, outcome, per step: role, model id, prompt version, input, output and cache-read tokens, elapsed; the selected-entity list with scores; violation ids per iteration; fact ids promoted, pending, rejected. The record is written after every step, not only at the end, so a crash leaves the steps completed so far on disk (FR-TURN-09). |
