...
(status `approved`, re-approved 2026-09-24). The spec says *what* and *why*; this file says *how* and
...
>
> **Returned to `draft` on 2026-09-24, with the spec, and re-approved the same day**
> (Process 2, rule 10): approved by the agent on the user's explicit delegation, 2026-09-24.
> No human has reviewed this revision. Promotion is add-only and the canoniser no longer looks
> for collisions (spec FR-OPS-06, FR-AGENT-05, AC 13, AC 20); what changed here: C15, the
> step 18 text, the verification rows for AC 13 and AC 20, three coverage rows and one risk.
...
Paths this list got wrong or left out, and decisions amended while implementing. C1-C14
change neither the scope, the steps nor an acceptance criterion, so the plan stayed
`approved`; C15 follows a spec revision that changed AC 13 and AC 20, and went through
`draft` with it (see the note at the top). They
are recorded here rather than applied quietly, because a file list that disagrees with the
tree is the thing this section exists to prevent.
...
| C9 | Step 15's model client, beyond P14: `ModelCallFailed` (502) added to `commons/errors/` for FR-LLM-08's failures (IF-07 clarified in `8cbee0f`); the npm shim `which` finds on Windows is resolved to the native `claude.exe` it forwards to and is never run, and `CLAUDE_CLI` overrides the lookup; `CLAUDE_TIMEOUT_SECONDS` in `config.py` and `.env.example`; `commons/llm/claude_code_client.py` exempted **by file** from forbidden-store-write, in the YAML and the mirror, for the temporary `--system-prompt-file`; the other `commons/llm/tests/` modules import the subprocess recorder from `test_claude_code_command.py`, since no conftest is listed | P14 named `shutil.which("claude")`, which on Windows returns a `.CMD` batch file: running it goes through `cmd.exe`, which re-parses the arguments. A temporary file is not a store path, like `.index/`. |
| C10 | **Decision P8 amended.** `semgrep` 1.177 installs and runs natively on Windows through `uvx`, so both gate scripts run it, pinned: `semgrep --test` against the fixtures, then a scan of `app/`. The fixtures move from `semgrep/tests/{positive,negative}/` to one annotated file per rule file, `semgrep/tests/<rule file>.py`, in semgrep's test format; `tests/test_boundaries_mirror.py` holds the mirror to the same annotated lines. Running the rules for the first time showed them wrong: the AC 13 rule matched nothing (`"canon/..."` is a literal string to semgrep, not a prefix) and the literal form it aimed at is the one the code rules already forbid, so it now refuses canon and cast paths, path shapes it cannot check, and the canon and cast services' write functions outside `promote`/`rule` (`reconcile`, which never writes, lost its exemption); the AC 3 rules gained `.open()` with no arguments, single-quoted f-strings and `+` concatenation, and the AC 17 rule set literals, all of which the mirror already caught | P8's premise was measured false on 2026-09-23. A rule of record that nothing ever ran is a rule nobody knows is right, and this one was not. |
| C11 | Files steps 9 and 14 added that the list did not name: `app/commons/db/tests/{conftest,support}.py` (the app with the fake embedder, and helpers the db suites share); `app/ledger/audit/{findings,material,persist,report,runner}.py` (the finding vocabulary, the loading, the merge into `ledger/violations.yaml`, the report and the entry point that the eight invariant modules share); `app/ledger/tests/test_audit_table.py` (the fixture README's whole table at once) and `app/scenes/tests/test_audit_route.py` (the route lives in `scenes`, which owns `/scenes/{id}/...`) | One module per invariant (step 14) leaves what they share needing a home inside the package; the rest are tests in the feature that owns what they test. |
| C12 | FR-PERM-07's fixed "documents are data" statement goes at the end of the role's system prompt file rather than at the head of stdin; the statement and the document delimiters are protocol, not the useful context FR-CTX-01 caps, so they stay out of the estimate, absorbed by the pessimistic divisor and visible through `over_cap` if they were not. The root `fixture_client` serves the app with `FakeEmbedder`, and `main.py` refuses to start offline over a missing model (FR-EMB-03), both in files already listed | FR-PERM-07 says "a fixed **system** instruction"; in the same message as the data it governs it had less authority than the spec gives it. |
| C13 | Steps 17-20, beyond the list: `app/agents/tests/test_roles.py` (role unit tests); per-role pruning (FR-CTX-03) lives in the role functions of step 17, since each role's mandatory/prunable split is its input contract, and step 18 records it; `assemble_context` gains `role_inputs` (the scene record, and for revise the draft and blocking findings, counted as mandatory); the full audit route receives the semantic auditor by dependency inversion through `commons/deps.py`, wired in `main.py`; the scene digest runs after the polish re-check; setups reach the writer as documents under *may collect*, so the AC 23 row's "the writer's instruction offers setups" is checked on the documents; `conftest.py` replaces the real CLI runner in every non-`live` test; `app/commons/embeddings/tests/test_fastembed.py` supplies AC 9's real half, left out at step 8; `tests/live/support.py` holds what the two live tests share | Each keeps a rule the plan already had in a place the code could honour it; none changes a step's scope or a criterion. |
| C14 | Step 20's live runs, beyond the list, after the spec's revision of DR-07 and its clarifications of FR-OPS-06, FR-AGENT-01, -05 and -06: `app/commons/schemas/violation.py` gains `explanation`, with `schemas/violations.v1.json` and `openapi.json` regenerated; `app/agents/roles/auditor.py` stores the model's explanation on each finding, `app/ledger/audit/persist.py` refreshes it when a re-audit reproduces the finding, and `app/agents/roles/writer.py` and `prompts/writer.md` hand it to `revise`; `app/ledger/promote.py` exposes what a fact may target (`promotable_targets`, `field_shape`, `split_mapping_payload`), re-exported by `app/ledger/service.py`, and compares a body fact with the as-of body; `app/cast/service.py` gains `body_at`, which the auditor's participant body and `promote` share; `app/agents/roles/targets.py` (new) renders the address list the canoniser and the writer both receive; `app/agents/models.py` gains `ExtractCall` and `RejectedFact`; `tests/live/test_audit_live.py` (new) audits a hand-planted draft of the tempting scene, so AC 26's auditor half is shown even when an obedient writer leaves nothing to flag; `tests/live/support.py` prints one progress line per step; `tests/live/last_run.md` is committed as the runs' evidence; `conftest.py` skips `live` tests by marker rather than by keyword, so the offline checks under `tests/live/` run | The first live runs of AC 26 and AC 27 missed for reasons in the code, not only in the prompts: extraction named fields no record has, and a registered body change collided with the pre-change record. The fixes stay inside the steps and criteria; the spec records the reading they rest on, and the one new field went through a spec revision first. |
| C15 | **Add-only promotion (spec revision of 2026-09-24; supersedes C14's as-of-body comparison for `promote`).** `app/ledger/promote.py`: `promote` gets its own pure add-only computation (empty field set; filled text field appended as `existing.rstrip() + "; " + payload.strip()` unless its words are already there, in order; list appended unless present; a missing mapping key added; a stored key, a key any registered ChangeEvent names, or a filled identifier or name never changed, the fact settled `rejected` with no ruling), its return type narrowed to `Promoted`, `conflict` and `existing_value` never set or cleared; today's computation, `held` and the as-of body kept for `rule` only, and the canon write stays lexically inside `promote` and `rule`; the normaliser lives here and is re-exported by `app/ledger/service.py`, and `canoniser.normalise_payload` is not changed, since it keys `proposal_id`. `app/agents/turn.py`: `_promote` without the collision and Escalation branches, reconcile only on `promoted`, the reason code `already_specified field=<target_field>` on a not-applied fact (and on the mirror branch), outcome always `merged`; `_refuse_pending_collisions` removed, the lock's 409 kept. `app/commons/schemas/turn.py`: the `FactRecord.settled` code loses its conflict clause (docstring and fields unchanged). `app/agents/roles/targets.py`, `app/agents/roles/canoniser.py`, `prompts/canoniser.md`, `prompts/writer.md`: propose only what the records do not specify, leave out contradictions, a filled text field gets only the new detail. Docstrings and messages only: `app/ledger/models.py` (module), `app/commons/deps.py`, `app/cast/service.py` (`body_at`), `tools/check_boundaries.py` and `semgrep/canon-write-outside-promote.yaml` (message text, never patterns). Tests: `app/ledger/tests/test_promote.py` (rewritten to add-only, with the property, crash-resume, word-boundary, ChangeEvent-key and legacy `conflict: true` cases; the `rule` tests kept on hand-built legacy state), `app/agents/tests/test_turn_ruling.py` (AC 20's add-only turn cases, including a resumed legacy record ending `merged`; the FR-TURN-08 rulings tests kept on a hand-built `awaiting_ruling` record; the pending-ruling 409 test removed), `app/agents/tests/test_turn_escalation.py` (AC 20 tag on the lock test), `app/agents/tests/test_roles.py` (canoniser prompt: the old contradiction instruction absent, the leave-out rule present, AC 27), `app/agents/tests/scripts/collision_002.yaml` (header only), `tests/live/test_turn_live.py` (`merged` only, AC 26), `tests/fixtures/repo/README.md` (prose on the removed gate only). Not changed: `openapi.json`, `schemas/*.json`, every emitted docstring and description (the spec lists the now-stale ones as deferred), fixture data files, `frontend/`. | The user ordered on 2026-09-24 that the canoniser stop checking collisions, since the auditor judges prose against canon before the draft is accepted. Unlike C1-C14 this one changes AC 13 and AC 20 (letters kept) and AC 26's wording, so the plan went back to `draft` with the spec and was re-approved by the agent on the user's explicit delegation. The contract is kept byte-identical because the `frontend/` session regenerates its types from it. |
...
| 11 | `backend:` | `select_entities` with BM25 + cosine fused by reciprocal rank, pins first, POV excluded, after the FR-IDX-08 incremental update (spec clarification `8cbee0f`); `/scenes/{id}/select`. | AC 11 |
...
| 18 | `backend:` | Turn orchestrator: state machine, lock, turn records after each step, hand-off **through the stores** (audit reads the draft back from `manuscript/`, revise reads blocking violations back from `ledger/violations.yaml`; the orchestrator passes ids only), revise scope guard, extraction on the accepted draft, add-only promotion followed by `reconcile` on each promoted target (a fact not applied is recorded with its reason code and the turn still ends `merged`; C15), `words` vs `budget` and digest length vs level target on the record, chapter-complete hint, `awaiting_ruling` and `rulings` kept in the contract but reached by no v1 turn (the route still runs `reconcile` on `accept`; C15), `resume`, SSE progress, `dry_run`, per-role pruning (FR-CTX-03/-04) with removed ids and `truncated_at` on the record. Scenario tests (happy, escalation after 3, revise rejected, add-only promotion with a fact not applied, rulings on a hand-built legacy record, malformed, refusal, budget, resume, provenance), plus: after a merged turn, a `dry_run` of the next scene in discourse order contains the promoted fact ("canon is updated before the next scene is assembled"); the fake call log shows the revise step's violations equal to the file's content and each role's documents inside its `INPUT_TABLE` row. Cross-feature test that writer and auditor receive the same selected list. | AC 12 (turn half), 18–24, 32, 33; FR-AGENT-09, FR-AGENT-11, FR-TURN-04 |
...
| 3 | A | `semgrep --test` holds both rules to the annotated lines of `semgrep/tests/forbidden-store-write.py`; `semgrep scan` of `app/` clean; the AST mirror flags exactly the same annotated lines (C10) | `semgrep/forbidden-store-write.yaml`, `tests/test_boundaries_mirror.py` |
...
| 13 | T, A | Add-only promote (C15), every result `isinstance(..., Promoted)`: empty field set; filled text field appended, the new value checked exactly (old text a prefix), never by `payload in new`; payload whose words are already there in order → `promoted`, no write (case and punctuation variants included; "red" next to "reddish" is appended); list appended unless present; missing mapping key added; a stored key with a **different** value, a key only a registered ChangeEvent names, and a filled identifier or name → `rejected` with `ruling` None, `conflict` false, `existing_value` None, target byte-identical, changed files exactly {`ledger/proposed.yaml`}; crash-resume (canon already holding the append, key or id, fact pending) → `promoted`, no second write; a payload with no word token → `InvalidRecord`; a legacy `conflict: true` pending fact promoted add-only with both fields kept; `hypothesis` property over every promotable field of every fixture record: always `Promoted`, changed files within {target, `ledger/proposed.yaml`}, old scalar a prefix, old list a prefix, old map a sub-map with equal values; `rule` refuses actor `agent` (kept tests, `test_an_agent_may_not_rule`) and `test_every_canon_or_cast_write_comes_from_promote_or_rule` kept; `semgrep --test` + mirror against `semgrep/tests/canon-write-outside-promote.py`: under `ledger/`/`agents/`, outside `promote`/`rule`, no store write aimed at canon or cast, none whose path cannot be shown not to be, and no call to the canon or cast services' write functions (C10) | `ledger/tests/test_promote.py`, `semgrep/canon-write-outside-promote.yaml` |
...
| 17 | T, A | Enumerate `toolset_for(role)`: writer has no `canon/**`, auditor only `ledger/violations.yaml`, canoniser nothing under `manuscript/`; `semgrep --test` + mirror against `semgrep/tests/hand-written-toolset.py`: no literal list, tuple or set of store paths in `agents/` | `commons/permissions/tests/test_toolsets.py`, `semgrep/hand-written-toolset.yaml` |
...
| 20 | T | Extraction including an already-specified key → turn `merged`, that `FactRecord` `rejected` (not `promoted`) with reason `already_specified field=<target_field>`, `conflict` false, no reconcile for it, record byte-identical; extraction aimed at a filled text field → the detail appended, turn `merged`; a legacy `conflict: true` pending fact does not block `begin_turn` on its scene, `FactRecord(conflict=True, status=pending).settled` is False, and a resumed legacy record ends `merged`; second turn while one runs → `409` by the lock (`test_a_second_turn_while_one_runs_is_refused`, tagged AC 20; the pending-ruling 409 test is removed). FR-TURN-08 rulings stay covered on a hand-built `awaiting_ruling` record (C15) | `agents/tests/test_turn_ruling.py`, `agents/tests/test_turn_escalation.py` |
...
| L4 · ProposedFact | Promotion add-only: never overwrites, an already-specified value not applied | AC 13, 20 (C15) | Covered |
...
| Operations | `promote` add-only, never escalates | AC 13, 20 (C15) | Covered |
...
| Figure 3 | Canoniser adds only what the records do not specify; no human gate on promotion | AC 13, 20, 27 (C15); the unreviewed new fact is the U entry in `verification.md` | Covered |
...
- Add-only promotion (C15) turns out to need to replace a value a record already holds, or
  to write any file other than the target record and `ledger/proposed.yaml`. No human gate
  catches it any more, so it is a spec change, not a code fix.
