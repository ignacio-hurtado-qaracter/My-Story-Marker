...
// data/books.js is regenerated by scripts/build-data.mjs (run by a Claude Code hook after
// every file write, and live by scripts/serve.mjs). The page re-fetches it every few seconds
// with a cache-busting query and re-renders only when the content actually changed.
const POLL_MS = 5000;
let signature = null;
let lastLoadedAt = null;

function fingerprint(data) {
  return JSON.stringify((data.books ?? []).map((b) => [b.slug, b.status, b.updatedAt, b.stats?.verdicts, b.stats?.words]));
}

function fetchData() {
  // Loading as a classic <script> works over file:// as well as HTTP.
  return new Promise((resolve, reject) => {
    const prev = window.__BOOKS__;
    const s = document.createElement('script');
    s.src = `data/books.js?t=${Date.now()}`;
    s.onload = () => { s.remove(); resolve(window.__BOOKS__ ?? prev); };
    s.onerror = () => { s.remove(); reject(new Error('no data/books.js')); };
    document.head.appendChild(s);
  });
}

function applyData(data, { announce = false } = {}) {
  const sig = fingerprint(data);
  lastLoadedAt = new Date();
  updateLiveStatus(false);
  if (sig === signature) return false;
  const isFirst = signature === null;
  const before = new Set(books.map((b) => b.slug));
  signature = sig;
  books = data.books ?? [];
  listNote.textContent = books.length ? `${books.length} novela(s) · datos generados ${fmtDate(data.generatedAt)}` : '';
  renderList();
  route();
  if (announce && !isFirst) {
    const added = books.filter((b) => !before.has(b.slug)).map((b) => b.title);
    showToast(added.length ? `Nueva novela: ${added.join(', ')}` : 'Biblioteca actualizada');
  }
  return true;
}
...
async function load() {
  try {
    const data = window.__BOOKS__ ?? (await fetchData());
    applyData(data);
  } catch (e) {
...
async function poll({ announce = true } = {}) {
  ...
  polling = true;
  try {
    applyData(await fetchData(), { announce });
  } catch {
...
function updateLiveStatus(stale) {
  const dot = document.querySelector('.live-dot');
  const text = document.getElementById('live-text');
  if (!dot || !text) return;
  dot.classList.toggle('stale', stale);
  const when = lastLoadedAt ? lastLoadedAt.toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit', second: '2-digit' }) : '—';
  text.textContent = stale
    ? `Sin acceso a los datos · último éxito ${when}`
    : `Actualización automática cada ${POLL_MS / 1000} s · última comprobación ${when}`;
}

document.getElementById('btn-refresh')?.addEventListener('click', async () => {
  const changed = await (async () => { try { return applyData(await fetchData(), { announce: true }); } catch { updateLiveStatus(true); return false; } })();
  if (!changed) showToast('La biblioteca ya está al día');
});
